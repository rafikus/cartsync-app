...
export function Header({
  ...
  return (
    <View style={[styles.wrapper, { backgroundColor: c.bgSurface, paddingTop: insets.top + spacing.sm }]}>
      {/* Status row */}
      ...
      <View style={styles.mainRow}>
        {onBack ? (
          <Pressable
            onPress={onBack}
            hitSlop={10}
            style={[styles.backBtn, { backgroundColor: c.bgSubtle, borderColor: c.borderDefault }]}
          >
            <Text style={[styles.backArrow, { color: c.textSecondary }]}>‹</Text>
          </Pressable>
        ) : (
        ...
        <View style={styles.rightSlot}>
          {badge ? (
            <View style={[styles.badge, { backgroundColor: c.success }]}>
              <Text style={[styles.badgeText, { color: c.successText }]}>{badge}</Text>
            </View>
          ) : null}
          {partner ? (
            <Avatar
...
const styles = StyleSheet.create({
  ...
  backBtn:         { width: 30, height: 30, borderRadius: radius.md, borderWidth: 0.5, alignItems: 'center', justifyContent: 'center', flexShrink: 0 },
  backArrow:       { fontSize: 20, marginTop: -1 },
  backPlaceholder: { width: 30 },
  titleBlock:      { flex: 1, minWidth: 0 },
  title:           { fontSize: textSizes['2xl'], fontWeight: '500', lineHeight: textSizes['2xl'] * 1.2 },
  subtitle:        { fontSize: textSizes.sm, marginTop: 2 },
  rightSlot:       { flexDirection: 'row', alignItems: 'center', gap: spacing.sm, flexShrink: 0, paddingTop: 3 },
  badge:           { borderRadius: radius.sm, paddingVertical: 3, paddingHorizontal: spacing.sm },
