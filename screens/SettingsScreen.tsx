...
export function SettingsScreen({
  navigation,
}: NativeStackScreenProps<AppStackParams, "Settings">) {
  const c = useColors();
  const insets = useSafeAreaInsets();
  const { user, logout } = useAuth();
  const { list, stores, syncState, clearList } = useList();
  ...
  const handlePressIn = () => {
    pressTimer.current = setTimeout(
      () => Clipboard.setStringAsync(list?.inviteCode ?? ""),
      400,
    );
  };
  ...
  const shareCode = () => {
    if (list?.inviteCode) {
      Share.share({
        message: `Join my CartSync list with code: ${list.inviteCode}`,
      });
    }
  };
  ...
  return (
    <View style={[{ flex: 1 }, { backgroundColor: c.bgApp }]}>
      ...
      >
        ...
        >
          <Text style={[s.cardTitle, { color: c.accentText }]}>
            Switch list
          </Text>
          <Text style={[s.cardSub, { color: c.textTertiary }]}>
            Open or create another list
          </Text>
        </Pressable>
        ...
        >
          ...
          <Text
            style={[
              s.cardSub,
              { color: c.textTertiary, marginBottom: spacing.md },
            ]}
          >
            Share this to invite your partner
          </Text>
          <Pressable
            style={[
              s.codeBox,
          ...
          >
            <Text style={[s.codeText, { color: c.text }]}>
              {list?.inviteCode ?? "–"}
            </Text>
          </Pressable>
          <Pressable
            onPress={shareCode}
            style={[s.shareBtn, { backgroundColor: c.accent }]}
          >
            <Text
              style={{
                color: "#fff",
          ...
          stores.map((store) => (
            ...
            >
              ...
              <Text style={[s.cardSub, { color: c.textTertiary }]}>
                ...
                {store.tripCount >= 2 ? "order learned" : "still learning"}
              </Text>
            </View>
          ))
        )}
