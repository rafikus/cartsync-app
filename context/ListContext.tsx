...
import {
  listsApi, itemsApi, storesApi, tripsApi, suggestionsApi,
  ListItem, ShoppingList, Store, Trip, Suggestion,
} from '../services/api';
...
interface ListCtx {
  ...
  updateItem:        (id: string, changes: Partial<Pick<ListItem, 'name' | 'quantity' | 'unit'>>) => Promise<void>;
...
export function ListProvider({ children }: { children: React.ReactNode }) {
  ...
  const [partnerCheckingId, setPartnerCheckingId] = useState<string | null>(null);
  ...
  useEffect(() => {
    ...
    return () => { unsub(); wsClient.onStatusChange = undefined; };
  }, []);
  ...
  const handleWs = useCallback((msg: WsMessage) => {
    switch (msg.type) {
      ...
      case 'item:updated':
        setItems((prev) =>
          prev.map((i) => i.id === (msg.payload as ListItem).id ? { ...i, ...(msg.payload as ListItem) } : i)
        );
        break;
      case 'item:removed':
        setItems((prev) => prev.filter((i) => i.id !== (msg.payload as { id: string }).id));
        break;
      case 'item:checking': {
        ...
        partnerTimer.current = setTimeout(() => setPartnerCheckingId(null), 2000);
        break;
      }
      case 'list:updated':
        setList((prev) => prev ? { ...prev, ...(msg.payload as Partial<ShoppingList>) } : prev);
        break;
      case 'store:updated':
        setStores((prev) =>
          prev.map((s) => s.id === (msg.payload as Store).id ? { ...s, ...(msg.payload as Store) } : s)
        );
        break;
      case 'trip:completed':
  ...
  const addItem = useCallback(async (name: string, qty: number, unit: string) => {
    if (!listIdRef.current) return;
  ...
  const toggleItem = useCallback(async (id: string) => {
    ...
    if (nowChecked) wsClient.send({ type: 'item:checking', payload: { itemId: id } });
    // Optimistic
    setItems((prev) => prev.map((i) => i.id === id ? { ...i, checked: nowChecked } : i));
    try {
      await itemsApi.update(listIdRef.current, id, { checked: nowChecked });
    } catch {
      // Rollback
      setItems((prev) => prev.map((i) => i.id === id ? { ...i, checked: current.checked } : i));
    }
  }, [items]);

  const updateItem = useCallback(async (id: string, changes: Partial<Pick<ListItem, 'name' | 'quantity' | 'unit'>>) => {
    if (!listIdRef.current) return;
  ...
  const completeTrip = useCallback(async (storeId?: string, order?: string[]): Promise<Trip> => {
  ...
  return (
    <Ctx.Provider value={{
      list, items, stores, trips, suggestions,
      syncState, partnerCheckingId, loading, error,
      loadList, addItem, toggleItem, updateItem, removeItem,
      completeTrip, loadSuggestions,
    }}>
      {children}
    </Ctx.Provider>
  );
